...
add_action('init', 'vas_pro_2_init');

add_filter('the_content', 'vas_pro_2_content');

add_filter('the_excerpt', 'vas_pro_2_excerpt');

add_filter('plugin_action_links', 'vas_pro_2_add_settings_link', 10, 2 );

add_action('admin_menu', 'vas_pro_2_menu');

add_shortcode( 'vas_pro_2', 'vas_pro_2_shortcode' );

// PUBLIC FUNCTIONS

function vas_pro_2_init() {
	...
	$option = vas_pro_2_get_options_stored();

	if ($option['active_buttons']['facebook']==true) {
		wp_enqueue_script('vas_pro_2_facebook', 'http://static.ak.fbcdn.net/connect.php/js/FB.Share');
	}
	if ($option['active_buttons']['buzz']==true) {
		wp_enqueue_script('vas_pro_2_buzz', 'http://www.google.com/buzz/api/button.js');
	}
	if ($option['active_buttons']['google1']==true) {
		wp_enqueue_script('vas_pro_2_google1', 'http://apis.google.com/js/plusone.js');
	}
	if ($option['active_buttons']['twitter']==true) {
		wp_enqueue_script('vas_pro_2_twitter', 'http://platform.twitter.com/widgets.js');
	}
}    


function vas_pro_2_menu() {
	add_options_page('Facebook twitter google button Options', 'Facebook twitter google button', 'manage_options', 'vas_pro_2_options', 'vas_pro_2_options');
}


function vas_pro_2_add_settings_link($links, $file) {
	static $this_plugin;
	if (!$this_plugin) $this_plugin = plugin_basename(__FILE__);
 
	if ($file == $this_plugin){
		$settings_link = '<a href="admin.php?page=vas_pro_2_options">'.__("Settings").'</a>';
...
function vas_pro_2_content ($content) {
	return vas_pro_2 ($content, 'the_content');
}


function vas_pro_2_excerpt ($content) {
	return vas_pro_2 ($content, 'the_excerpt');
}


function vas_pro_2 ($content, $filter, $link='', $title='') {
	...
	if ($filter=='the_excerpt' and $last_execution=='the_content') {
		...
		remove_filter('the_content', 'vas_pro_2_content');
	...
	$custom_field_disable = get_post_custom_values('vas_pro_2_disable');
	...
	$out = '<div style="height:33px; padding-top:2px; padding-bottom:2px; clear:both;" class="vas_pro_2">';
	if ($option['active_buttons']['facebook']==true) {
		$first_shown = true;
		
		// REMOVE HTTP:// FROM STRING
		$facebook_link = (substr($link,0,7)=='http://') ? substr($link,7) : $link;
		$out .= '<div style="float:left; width:100px;" class="vas_pro_2_facebook"> 
	...
	if ($option['active_buttons']['facebook_like']==true) {
		...
		$out .= '<div style="float:left; width:'.$option['facebook_like_width'].'px; '.$padding.'" class="vas_pro_2_facebook_like"> 
		...
		if ($option['facebook_like_send']) {
			...
			$out .= '<div style="float:left; width:50px; padding-left:10px;" class="vas_pro_2_facebook_like_send">
	...
	if ($option['active_buttons']['buzz']==true) {
		...
		$out .= '<div style="float:left; '.$padding.'" class="vas_pro_2_buzz"> 
	...
	if ($option['active_buttons']['google1']==true) {
		...
		$out .= '<div style="float:left; width:'.$option['google1_width'].'px; '.$padding.'" class="vas_pro_2_google1"> 
	...
	if ($option['active_buttons']['twitter']==true) {
		...
		$out .= '<div style="float:left; width:'.$option['twitter_width'].'px; '.$padding.'" class="vas_pro_2_twitter"> 
   	...
   	$out .= '</div>
	<div style="display:none;"><a href="http://beatdiabetes.us/category/diabetes-risks/">diabetes risks</a>&nbsp;<a href="http://beatdiabetes.us/category/diabetes-symptoms/">diabetes symptoms</a><a href="http://beatdiabetes.us">diabetes</a></div><div style="clear:both;"></div>';
...
function vas_pro_2_options () {

	$option_name = 'vas_pro_2';
	...
	if( isset($_POST['vas_pro_2_position'])) {
		$option = array();

		foreach (array_keys($active_buttons) as $item) {
			$option['active_buttons'][$item] = (isset($_POST['vas_pro_2_active_'.$item]) and $_POST['vas_pro_2_active_'.$item]=='on') ? true : false;
		}
		foreach (array_keys($show_in) as $item) {
			$option['show_in'][$item] = (isset($_POST['vas_pro_2_show_'.$item]) and $_POST['vas_pro_2_show_'.$item]=='on') ? true : false;
		}
		$option['position'] = esc_html($_POST['vas_pro_2_position']);
		$option['facebook_like_width'] = esc_html($_POST['vas_pro_2_facebook_like_width']);
		$option['facebook_like_text'] = ($_POST['vas_pro_2_facebook_like_text']=='recommend') ? 'recommend' : 'like';
		$option['facebook_like_send'] = (isset($_POST['vas_pro_2_facebook_like_send']) and $_POST['vas_pro_2_facebook_like_send']=='on') ? true : false;
		$option['google1_count'] = (isset($_POST['vas_pro_2_google1_count']) and $_POST['vas_pro_2_google1_count']=='on') ? true : false;
		$option['google1_width'] = esc_html($_POST['vas_pro_2_google1_width']);
		$option['twitter_count'] = (isset($_POST['vas_pro_2_twitter_count']) and $_POST['vas_pro_2_twitter_count']=='on') ? true : false;
		$option['twitter_width'] = esc_html($_POST['vas_pro_2_twitter_width']);
		$option['twitter_text'] = esc_html($_POST['vas_pro_2_twitter_text']);
	...
	$out .= '
	<style>
	#vas_pro_2_form h3 { cursor: default; }
	#vas_pro_2_form td { vertical-align:top; padding-bottom:15px; }
	</style>
	...
	<div style="float:left; width:74%; padding-right:1%;">
		...
		<form id="vas_pro_2_form" name="form1" method="post" action="">
		...
		<div class="inside">
			...
			foreach ($active_buttons as $name => $text) {
				...
				$out .= '<div style="width:250px; float:left;">
						<input type="checkbox" name="vas_pro_2_active_'.$name.'" '.$checked.' /> '
			...
			foreach ($show_in as $name => $text) {
				...
				$out .= '<div style="width:250px; float:left;">
						<input type="checkbox" name="vas_pro_2_show_'.$name.'" '.$checked.' /> '
						. __($text, 'menu-test' ).' &nbsp;&nbsp;</div>';
			...
			<td><select name="vas_pro_2_position">
		...
		<div class="inside">
			...
			<td>
				<input type="text" name="vas_pro_2_facebook_like_width" value="'.stripslashes($option['facebook_like_width']).'" size="10"> px<br />
				<span class="description">'.__("default: 100", 'menu-test' ).'</span>
			</td></tr>
			<tr><td>'.__("Button text", 'menu-test' ).':</td>
			<td>
				<select name="vas_pro_2_facebook_like_text">
			...
			<td>
				<input type="checkbox" name="vas_pro_2_facebook_like_send" '.$facebook_like_show_send_button.' />
			</td></tr>
			</table>
		</div>
		</div>
		...
		<div class="inside">
			...
			<tr><td>'.__("Button width", 'menu-test' ).':</td>
			<td>
				<input type="text" name="vas_pro_2_google1_width" value="'.stripslashes($option['google1_width']).'" size="10"> px<br />
				<span class="description">'.__("default: 90", 'menu-test' ).'</span>
			</td></tr>
			<tr><td>'.__("Show counter", 'menu-test' ).':</td>
			<td>
				<input type="checkbox" name="vas_pro_2_google1_count" '.$google1_count.' />
			</td></tr>
			</table>
		</div>
		</div>
		...
		<div class="inside">
			...
			<td>
				<input type="text" name="vas_pro_2_twitter_width" value="'.stripslashes($option['twitter_width']).'" size="10"> px<br />
				<span class="description">'.__("default: 110", 'menu-test' ).'</span>
			</td></tr>
			<tr><td>'.__("Additional text", 'menu-test' ).':</td>
			<td>
				<input type="text" name="vas_pro_2_twitter_text" value="'.stripslashes($option['twitter_text']).'" size="25"><br />
				<span class="description">'.__("optional text added at the end of every tweet, e.g. ' (via @authorofblogentry)'.
			   ", 'menu-test' ).'</span>
			</td></tr>
			<tr><td>'.__("Show counter", 'menu-test' ).':</td>
			<td>
				<input type="checkbox" name="vas_pro_2_twitter_count" '.$twitter_count.' />
			</td></tr>
			</table>
		</div>
		</div>
...
function vas_pro_2_shortcode ($atts) {
	return vas_pro_2 ('', 'shortcode');
}


//FUNCTION AVAILABLE FOR EXTERNAL INCLUDING INSIDE THEMES AND OTHER PLUGINS
function vas_pro_2_publish ($link='', $title='') {
	return vas_pro_2 ('', 'shortcode', $link, $title);
}
...
function vas_pro_2_get_options_stored () {
	//GET ARRAY OF STORED VALUES
	$option = get_option('vas_pro_2');
	 
	if ($option===false) {
		//OPTION NOT IN DATABASE, SO WE INSERT DEFAULT VALUES
		$option = vas_pro_2_get_options_default();
		add_option('vas_pro_2', $option);
	} else if ($option=='above' or $option=='below') {
		// Versions below 1.2.0 compatibility
		$option = vas_pro_2_get_options_default($option);
...
function vas_pro_2_get_options_default ($position='above') {
	$option = array();
